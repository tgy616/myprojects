...
import com.tgy.clouddisk.constant.MenuConsts;
import com.tgy.clouddisk.dao.req.MoveReq;
import com.tgy.clouddisk.dao.req.RenameFileReq;
import com.tgy.clouddisk.dao.req.RenameFolderReq;
import com.tgy.clouddisk.dao.req.ShredReq;
...
import com.tgy.clouddisk.service.DiskService;
...
public class HomeController {
    ...
    public Map<String, Object> move(@PathVariable String username, @RequestBody @Valid MoveReq req) {
        // int userId = userService.getUser(username).getId();
        // TODO 参数校验
        return diskService.move(req.getFolders(), req.getFiles(), req.getDest());
    }
    ...
    public UserFolderDTO renameFolder(@PathVariable String username, @PathVariable int folderId
            , @RequestBody @Valid RenameFolderReq req) {
        // int userId = userService.getUser(username).getId();
        // TODO 参数校验
        return diskService.renameFolder(folderId, req.getFolderName());
    }
    ...
    public UserFileDTO renameFile(@PathVariable String username, @PathVariable int fileId
            , @RequestBody @Valid RenameFileReq req) {
        // int userId = userService.getUser(username).getId();
        // TODO 参数校验
        return diskService.renameFile(fileId, req.getFileName(), req.getFileType());
    }
    ...
    public UserFolderDTO newFolder(@PathVariable String username, @RequestBody @Valid UserFolder req) {
        // TODO 参数校验
        int userId = userService.getUserByUsername(username).getUserId();
        req.setUserId(userId);

        return diskService.newFolder(req);
    }
    ...
    public UserDTO shred(@PathVariable String username
            , @RequestBody @Valid ShredReq req) {
        int userId = userService.getUserByUsername(username).getUserId();
        // TODO 参数校验
        return diskService.shred(req.getFolders(), req.getFiles(), userId);
    }
}